...
// ─────────────────────────────────────────────
// IndexedDB — armazenamento temporário do áudio (24h)
// ─────────────────────────────────────────────
...
async function saveRecordingToDB(blob) {
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      const req   = store.put({ blob, timestamp: Date.now() }, "latest");
      req.onsuccess = () => resolve();
      req.onerror   = (e) => reject(e.target.error);
    });
  } catch (err) {
    console.warn("IndexedDB: não foi possível salvar o áudio.", err);
  }
}
    ...
    <div style={{
      ...
      <button
        ...
        style={{
          ...
          color: "#B91C1C", fontSize: "18px", padding: 0,
...
export default function App() {
  ...
  // IndexedDB — gravação salva
  const [savedRecording, setSavedRecording] = useState(null); // { blob, timestamp }
  ...
  // ── Carregar gravação salva no mount ──
  useEffect(() => {
    loadRecordingFromDB().then((record) => {
      if (record) setSavedRecording(record);
    });
  }, []);
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        // Salvar no IndexedDB para recuperação em caso de falha
        await saveRecordingToDB(blob);
        setSavedRecording(null); // já está carregado, não precisa do banner
      };
  ...
  const transcribe = async () => {
    ...
    try {
      ...
      // Sucesso: limpar gravação salva do IndexedDB
      if (audioSource === "recording") {
        clearRecordingFromDB();
      }
    } catch (err) {
      setError(err.message);
      // Manter áudio no IndexedDB para nova tentativa — não limpar aqui
    } finally {
    ...
    <>
      ...
      <div style={{
        ...
        <main style={{ maxWidth: "600px", margin: "0 auto", padding: "8px 16px 64px" }}>
          ...
          {/* ── Banner: gravação salva recuperável ── */}
          {savedRecording && (
            <div
              className="fade-in"
              style={{
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                ...
                <button
                  ...
                  aria-label="Descartar gravação salva"
                  style={{
                    padding: "7px 10px", borderRadius: "8px",
                    border: "1px solid #FCD34D", background: "transparent",
                    color: "#92400E", fontSize: "12px", cursor: "pointer",
                    fontFamily: "inherit",
                  }}
                >
                  Descartar
          ...
          {!hasResult && (
            ...
            >
              {isTranscribing ? (
                <><span className="spin" style={{ fontSize: "18px" }}>◌</span>Transcrevendo...</>
              ) : (
                <>✨ Transcrever</>
              )}
            </button>
